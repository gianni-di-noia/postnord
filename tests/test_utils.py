...
class UtilsTest(TestCase):
    ...
    def test_template_caching_settings(self):
        """Check if POST_NORD_CACHE and POST_NORD_TEMPLATE_CACHE understood
        correctly
        """
        ...
        # disable cache
        with self.settings(POST_NORD_CACHE=False):
            is_cache_used(suffix='cache_disabled_global', desired_cache=False)
        with self.settings(POST_NORD_TEMPLATE_CACHE=False):
            is_cache_used(
                suffix='cache_disabled_for_templates', desired_cache=False
            )
        with self.settings(POST_NORD_CACHE=True, POST_NORD_TEMPLATE_CACHE=False):
            is_cache_used(
